	...
	private void commandQueue(MessageReceivedEvent event) {
		...
		synchronized (songQueue) {
			if (songQueue.isEmpty()) {
				...
				sb.append("Musica atual: " + guildManager.getAudioPlayer().getPlayingTrack().getInfo().title
						+ " | Lista: ").append(songQueue.size()).append("\n");
		...
		if (voiceChannel != null) {
			...
			try {
				...
				if (e.getPermission() == Permission.VOICE_CONNECT) {
					event.getChannel().sendMessage(
							"Eu não tenho permissão de entrar nesse canal de voz :( [" + voiceChannel.getName() + "].")
					.queue();
				}
			}
		}
