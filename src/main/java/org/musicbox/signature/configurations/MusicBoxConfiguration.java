...
public class MusicBoxConfiguration extends JsonConfiguration {
	...
	public String getFailedReason(String language, int select, List<Placeholder> placeholders) {
		return EmbedTranslator.translateCodes(getJsonObject("musicBoxMessages").get("failReasons")
				.getAsJsonObject().get(language).getAsJsonArray().get(select).getAsString(), placeholders);
	}
