...
public class SoundCloudAudioSourceManager implements AudioSourceManager, HttpConfigurable {
	...
	public SoundCloudAudioSourceManager(boolean allowSearch, SoundCloudDataReader dataReader,
			SoundCloudHtmlDataLoader htmlDataLoader, SoundCloudFormatHandler formatHandler,
	...
	private UserInfo findUserIdFromLikedList(HttpInterface httpInterface, String likedListUrl) throws IOException {
		try (CloseableHttpResponse response = httpInterface.execute(new HttpGet(likedListUrl))) {
			...
			Matcher matcher = likedUserUrnPattern
					.matcher(IOUtils.toString(response.getEntity().getContent(), StandardCharsets.UTF_8));
	...
	private AudioItem processAsSearchQuery(AudioReference reference) {
		if (reference.identifier.startsWith(SEARCH_PREFIX)) {
			if (reference.identifier.startsWith(SEARCH_PREFIX_DEFAULT)) {
				return loadSearchResult(reference.identifier.substring(SEARCH_PREFIX_DEFAULT.length()).trim(), 0,
			...
			if (searchMatcher.matches()) {
				return loadSearchResult(searchMatcher.group(3), Integer.parseInt(searchMatcher.group(1)),
						Integer.parseInt(searchMatcher.group(2)));
			}
		}
	...
	private AudioItem loadSearchResult(String query, int offset, int rawLimit) {
		...
		try (HttpInterface httpInterface = getHttpInterface();
				CloseableHttpResponse response = httpInterface
						.execute(new HttpGet(buildSearchUri(query, offset, limit)))) {
	...
	private URI buildSearchUri(String query, int offset, int limit) {
		try {
			return new URIBuilder("https://api-v2.soundcloud.com/search/tracks").addParameter("q", query)
					.addParameter("offset", String.valueOf(offset)).addParameter("limit", String.valueOf(limit))
					.build();
	...
	public static class Builder {
		...
		public SoundCloudAudioSourceManager build() {
			...
			if (usedPlaylistLoader == null) {
				usedPlaylistLoader = new DefaultSoundCloudPlaylistLoader(usedHtmlDataLoader, usedDataReader,
						usedFormatHandler);
			}

			return new SoundCloudAudioSourceManager(allowSearch, usedDataReader, usedHtmlDataLoader, usedFormatHandler,
					usedPlaylistLoader);
		}

		@FunctionalInterface
		interface PlaylistLoaderFactory {
			SoundCloudPlaylistLoader create(SoundCloudDataReader dataReader, SoundCloudHtmlDataLoader htmlDataLoader,
