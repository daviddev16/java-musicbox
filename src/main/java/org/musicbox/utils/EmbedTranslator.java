	...
	public static MessageEmbed translate(JsonObject jsonObject, List<Placeholder> placeholders) {
		...
		if (jsonObject.has("author")) {
			builder.setAuthor(
					...
					translateCodes(jsonObject.get("author").getAsJsonObject().get("iconUrl").getAsString(),
							placeholders));
		}
		if (jsonObject.has("footer")) {
			builder.setFooter(
					translateCodes(jsonObject.get("footer").getAsJsonObject().get("text").getAsString(), placeholders),
					translateCodes(jsonObject.get("footer").getAsJsonObject().get("iconUrl").getAsString(),
							placeholders));
		}
